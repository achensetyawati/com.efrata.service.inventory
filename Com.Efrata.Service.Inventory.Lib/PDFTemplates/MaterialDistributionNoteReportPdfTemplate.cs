...
namespace Com.Ambassador.Service.Inventory.Lib.PDFTemplates
{
    public class MaterialDistributionNoteReportPdfTemplate
    {
        public MemoryStream GeneratePdfTemplate(List<MaterialDistributionNoteReportViewModel> data, DateTime date, string unitName)
        {
            ...
            cb.ShowTextAligned(PdfContentByte.ALIGN_LEFT, "PT AMBASSADOR GARMINDO", 15, 820, 0);
            ...
            foreach (var d in data)
            {
                ...
                if (Number == TotalRows)
                {
                    cell = new PdfPCell() { Border = Rectangle.TOP_BORDER | Rectangle.LEFT_BORDER | Rectangle.BOTTOM_BORDER | Rectangle.RIGHT_BORDER, HorizontalAlignment = Element.ALIGN_RIGHT, VerticalAlignment = Element.ALIGN_MIDDLE, Colspan = 7, Phrase = new Phrase("Jumlah", normal_font), Padding = 2 };
                    table.AddCell(cell);
