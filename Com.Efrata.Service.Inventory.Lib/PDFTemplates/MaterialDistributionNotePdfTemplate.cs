        ...
        public MemoryStream GeneratePdfTemplate(MaterialDistributionNoteViewModel viewModel)
        {
            ...
            foreach (var item in viewModel.MaterialDistributionNoteItems)
            {
                foreach (var detail in item.MaterialDistributionNoteDetails)
                {
                    ...
                    if(Number == TotalRows)
                    {
                        cell = new PdfPCell() { Border = Rectangle.TOP_BORDER | Rectangle.LEFT_BORDER | Rectangle.BOTTOM_BORDER | Rectangle.RIGHT_BORDER, HorizontalAlignment = Element.ALIGN_RIGHT, VerticalAlignment = Element.ALIGN_MIDDLE, Colspan = 5, Phrase = new Phrase("Jumlah", normal_font), Padding = 5 };
                        table.AddCell(cell);
